    ...
    if (assetGuid !== undefined) {
      ...
      switch (sysTx.txtype) {
        case 'syscoinburntoassetallocation':
          ...
          //fix precision
          senderBalance = +(senderBalance).toFixed(8);
          console.log("syscoinburntoassetallocation:: Sender Balance:", senderBalance, 'address', address.a_id);

          //create inputs
          assetInputs.push({amount: senderBalance, address: sysTx.sender});
          ...
          console.log("syscoinburntoassetallocation:: SENDER CHANGE: change", senderChange, "balance", address.asset_allocation_balances[assetGuid], "sendsum", sum);
        ...
        case 'assetactivate':
          ...
          let balance = 0;
          if (sysTx.balance) {
            balance = parseFloat(sysTx.balance);
          }

          //create inputs
          assetInputs.push({amount: balance, address: 'coinbase'});

          //create outputs
          assetOutputs.push({amount: balance, address: sysTx.sender});
          break;
