...
const prepareAssetVinVout = async (hex) => {
  ...
  assetGuid = (await getAssetTxInfoFromHex(hex)).guid;
  const sysTx = await syscoinDecodeRawTransaction(hex);
    ...
    if (assetGuid !== undefined) {
      ...
      let sum = 0;
      switch (sysTx.txtype) {
        case 'assetsend': //assume sender has no allocations! TODO: check w jag

          const ownerAddress = await getAliasAddress(sysTx.owner);
          let address = await Address.findOne({a_id: ownerAddress});

          //create inputs, use db balance
          if (address && address.asset_balances[assetGuid] !== undefined) {
            assetInputs.push({amount: address.asset_balances[assetGuid], address: await getAliasAddress(sysTx.owner)});
          } else {
            throw new Error("Asset send without asset inputs! " + hex);
          }
          ...
          assetOutputs.push({amount: parseFloat(address.asset_balances[assetGuid] - sum), address: await getAliasAddress(assetInfo.alias)});
          break;
        ...
        case 'assetallocationsend':
          ...
          //create inputs
          assetInputs.push({amount: parseFloat(assetInfo.balance), address: await getAliasAddress(assetInfo.alias)});

          //create outputs
          for (let i = 0; i < sysTx.allocations.length; i++) {
            let allocation = sysTx.allocations[i];
            //console.log(JSON.stringify(allocation));
            assetOutputs.push({amount: parseFloat(address.asset_balances[assetGuid] - sum), address: await getAliasAddress(allocation.owner)});
            sum += parseFloat(allocation.amount);
          }
