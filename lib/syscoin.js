...
const prepareAssetVinVout = async (tx) => {
  ...
  if(sysTx) {
    ...
    if (assetGuid !== undefined) {
      console.log("ASSET PREPARE IN/OUT:", assetGuid, tx.txid);
      ...
      switch (sysTx.txtype) {
        case 'syscoinburntoassetallocation':
          ...
          for (let i = 0; i < sysTx.allocations.length; i++) {
            ...
            sum = sum.plus(new BigNumber(allocation.amount));
          }
        ...
        case 'assetallocationmint':
          ...
          for (let i = 0; i < sysTx.allocations.length; i++) {
            ...
            assetOutputs.push({amount: new BigNumber(allocation.amount).toFixed(), address: allocation.address});
            console.log("sum before adding: ", allocation.amount, new BigNumber(allocation.amount).toFixed());
            sum = sum.plus(new BigNumber(allocation.amount));
            console.log("new sum: ", sum.toFixed());
          }
