    ...
    if (assetGuid !== undefined) {
      ...
      switch (sysTx.txtype) {
        ...
        case 'assetallocationsend':
          ...
          if(sysTx.owner.trim() === "" && sysTx.txtype === "assetsend") {
            console.log(`Asset owner is blank, using assetinfo. Owner for ${assetInfo._id} is ${assetInfo.alias}`);
          ...
          if(!isFinite(sum) || isNaN(sum) || !isFinite(senderBalance) || isNaN(senderBalance)) {
            ...
            console.log("SUM:", sum);
            console.log("Sender Balancer:", senderBalance);
            process.exit(22);
          }
