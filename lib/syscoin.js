    ...
    if (assetGuid !== undefined) {
      ...
      switch (sysTx.txtype) {
        ...
        case 'assetallocationsend':
          ...
          let assetDbInfo = await Asset.findOne({asset_id: assetGuid});

          if(!sysTx.owner) {
            sysTx.owner = assetDbInfo.owner_address;
            console.log(`UNDEF OWNER Asset owner, using assetinfo. Owner for ${assetInfo._id} is ${sysTx.owner} txtype is ${sysTx.txtype}`);
          }else if(sysTx.owner.trim() === "") {
            sysTx.owner = assetDbInfo.owner_address;
            console.log(`BLANK OWNER Asset owner, using assetinfo. Owner for ${assetInfo._id} is ${sysTx.owner} txtype is ${sysTx.txtype}`);
          }
