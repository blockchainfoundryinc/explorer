...
const prepareAssetVinVout = async (tx) => {
  let assetGuid, assetInfo, assetAllocationInfo, senderAddress, address, assetDbInfo;
  ...
  if (assetGuid !== undefined) {
    ...
    switch (sysTx.txtype) {
      ...
      case 'assetallocationsend':
        if(!sysTx.owner) {
          console.log(`UNDEF OWNER Asset owner, using assetinfo. Owner for ${assetInfo._id} is ${assetDbInfo.owner_address} txtype is ${sysTx.txtype}`);
        }else if(sysTx.owner.trim() === "") {
          console.log(`BLANK OWNER Asset owner, using assetinfo. Owner for ${assetInfo._id} is ${assetDbInfo.owner_address} txtype is ${sysTx.txtype}`);
        }
        ...
        if (address && address.asset_balances[assetGuid] !== undefined) {
          ...
          assetInputs.push({ address: assetDbInfo.owner_address, amount: address.asset_balances[assetGuid]});
        }
      ...
      case 'assetupdate':
        ...
        if(sysTx.balance) {
          ...
          console.log("NO UPDATE DATA FOR", tx);
        }
        break;
