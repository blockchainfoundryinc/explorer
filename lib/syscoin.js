      ...
      switch (sysTx.txtype) {
        ...
        case 'assetallocationsend':
          ...
          if (address && address.asset_balances[assetGuid] !== undefined) {
            ...
            //pull input from vin
            let largest_vout = { address: "", amount: 0 };
            for(let vout in tx.vout) {
              if(largest_vout.amount < vout.value) {
                largest_vout = { address: vout.addresses[0], amount: vout.value };
              }
            }

            console.log("largest vout:", JSON.stringify(largest_vout));
            assetInputs.push(largest_vout);
          }
