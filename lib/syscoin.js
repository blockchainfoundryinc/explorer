...
const assetAllocationSend = async (assetGuid, senderAddress, receiverAddress, amount) => {
  try {
    ...
    console.log("ERR assetAllocationSend", JSON.stringify(e.response.data.error));
  }
};
...
const getRawTransactionBatch = async (keyObj) => {
  try {
    let requests = [];
    for (let [key, value] of Object.entries(keyObj)) {
      console.log(`${key}: ${value}`);
      requests.push(syscoinClient.callRpc("getrawtransaction", [key, 1]));
    }
    ...
    result.forEach(tx => {
      response[tx.txid] = tx.vout[ keyObj[tx.txid] ];
    });

    return response;
  }catch(e) {
    console.log("ERR getRawTransactionBatch", e);
  }
};
