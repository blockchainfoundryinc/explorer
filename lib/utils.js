...
async function buildAssetBalanceList(address) {
  ...
  for (let alloc_guid in address.asset_allocation_balances) {
    ...
    assetBalances[alloc_guid] = await buildAssetBalanceList(alloc_guid, assetInfo, address, numberWithCommas(address.asset_allocation_balances[alloc_guid]));
  }
  ...
  console.log('assetb', assetBalances);
