...
async function buildAssetBalanceList(address) {
  ...
  for (let alloc_guid in address.asset_allocation_balances) {
    ...
    assetBalances[alloc_guid] = createAssetBalance(alloc_guid, assetInfo, address, numberWithCommas(parseFloat(address.asset_allocation_balances[alloc_guid]), 8));
  }
  ...
  for (let guid in address.asset_balances) {
    if(assetBalances[guid]) {
      ...
      assetBalances[guid] = createAssetBalance(guid, assetInfo, address, 0, numberWithCommas(parseFloat(address.asset_balances[guid]), 8))
    }
  }
