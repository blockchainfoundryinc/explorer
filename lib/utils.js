const syscoinHelper = require('./lib/syscoin');
...
async function buildAssetBalanceList(address) {
  ...
  for (let alloc_guid in address.asset_allocation_balances) {
    ...
    assetBalances[alloc_guid] = buildAssetBalanceList(alloc_guid, assetInfo, address, utils.numberWithCommas(address.asset_allocation_balances[alloc_guid]));
  }
  ...
  for (let guid in address.asset_balances) {
    if(assetBalances[guid]) {
      assetBalances[guid].balance = utils.numberWithCommas(address.asset_balances[guid], 2)
    } else {
      assetBalances[guid] = createAssetBalance(guid, assetInfo, address, 0, utils.numberWithCommas(address.asset_balances[guid], 2))
    }
  }
