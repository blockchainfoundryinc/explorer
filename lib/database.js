...
function saveAssetInfo(tx, asset_guid, hash, cb) {
  if(tx.txtype === 'assetactivate' && asset_guid !== null) {
    console.log("ASSIGN OWNER:", hash, " TO ", asset_guid, " at height ", tx.height);
      ...
      lib.prepare_vin(tx, function (vin) {
        lib.prepare_vout(tx.vout, txid, vin, tx.hex, async function (vout, nvin) {
          ...
          let txtype = "send",
            ...
            asset_totalout = 0,
            asset_symbol;
          let assetTxInfo = await syscoinHelper.getAssetTxInfoFromHex(tx);

          if (assetTxInfo.guid) {
            txtype = assetTxInfo.txtype;
            asset_guid = assetTxInfo.guid;
            asset_symbol = assetTxInfo.symbol;
            asset_totalout = asset_vout.reduce((acc, curr, index, arr) => acc + curr.amount, 0);
            console.log("ASSET TX:", txid, " TXTYPE", txtype);
            console.log("asset total out: ", asset_totalout);
            console.log("asset vin/vout: ", assetInOut);

            //write asset history every time
            let assethist = new AssetHistory({
              asset_id: asset_guid,
              sender_address: tx.asset_vin.length ? tx.asset_vin[0].address : null,
              sender_alias: assetTxInfo.owner,
              txtxype: txtype,
              vin: tx.asset_vin,
              vout: tx.asset_vout,
              txdata: await syscoinHelper.getAssetTxInfoFromHex(tx),
              height: blockheight,
              txid: tx.txid
            });
            ...
            tx.txtype = txtype;
            tx.owner = assetTxInfo.owner;

            //detect asset doublespends and reject them
            if(tx.asset_vin.length && tx.asset_vout.length) {
              let asset_total_vout_not_change = 0;
              for(let i = 0; i < tx.asset_vout.length - 1; i++) {
                asset_total_vout_not_change += tx.asset_vout[i].amount;
              }

              if(asset_total_vout_not_change > tx.asset_vin[0].amount) {
                console.log("DOUBLE SPEND! ", tx.txid, "ASSET ", asset_guid);
                return cb();
              }
            }
          }
                ...
                lib.syncLoop(asset_vout.length, function (asset_vout_loop) {
                  ...
                  lib.calculate_total(vout, function (total) {
                    var newTx = new Tx({
                      ...
                      asset_total: asset_totalout.toFixed(8),
                      vout: vout,
