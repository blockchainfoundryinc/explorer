  ...
  lib.get_rawtransaction(txid, function(tx){
    if (tx != 'There was an error. Check your console.') {
      lib.get_block(tx.blockhash, function(block){
        if (block) {
          lib.prepare_vin(tx, function (vin, asset_vin) {
            lib.prepare_vout(tx.vout, txid, vin, asset_vin, tx.hex, async function (vout, nvin, asset_vout, nasset_vin) {
              let txtype = "send", asset_guid, asset_totalout = 0;
              let assetTxInfo = await syscoinHelper.getAssetGuidFromHex(tx.hex);
              if (assetTxInfo.guid) {
              ...
              lib.syncLoop(vin.length, function (tx_vin_loop) {
                ...
                //TODO update address on assettx
