...
function update_address(hash, txid, amount, type, asset_guid, tx, cb) {
  ...
  find_address(hash, function(address) {
    if (address) {
        ...
        lib.is_unique(address.txs, txid, function(unique, index) {
          ...
          switch(type) {
            ...
            case 'asset_vout':
              ...
              if(asset_balances[asset_guid] === undefined) {
                if(tx.systx.txtype === 'assettransfer' || tx.systx.txtype === 'assetactivate') {
                  asset_balances[asset_guid] = amount;
                }else if(tx.systx.txtype === 'assetallocationsend' || tx.systx.txtype === 'assetsend') {
                  asset_allocation_balances[asset_guid] = amount;
                }
              }else{
                if(tx.systx.txtype === 'assettransfer' || tx.systx.txtype === 'assetactivate') {
      ...
      if (type === 'vin') {
        ...
        let newAddressAssetBalances = { [asset_guid]: amount };
      ...
      lib.prepare_vin(tx, function (vin) {
        lib.prepare_vout(tx.vout, txid, vin, tx.hex, async function (vout, nvin) {
          ...
          lib.syncLoop(vin.length, function (tx_vin_loop) {
            ...
            update_address(nvin[i].addresses, txid, nvin[i].amount, 'vin', null, tx, function () {
              tx_vin_loop.next();
            });
          }, function () {
            lib.syncLoop(vout.length, function (tx_vout_loop) {
              ...
              if (vout[t].addresses) {
                update_address(vout[t].addresses, txid, vout[t].amount, 'vout', null, tx, function () {
                  tx_vout_loop.next();
                });
              } else {
                tx_vout_loop.next();
              ...
              lib.syncLoop(asset_vin.length, function (asset_vin_loop) {
                ...
                if (asset_vin[u].address) {
                  update_address(asset_vin[u].address, txid, asset_vin[u].amount, 'asset_vin', asset_guid, tx, function () {
                    asset_vin_loop.next();
                  });
                } else {
                  asset_vin_loop.next();
                ...
                lib.syncLoop(asset_vout.length, function (asset_vout_loop) {
                  ...
                  if (asset_vout[v].address) {
                    update_address(asset_vout[v].address, txid, asset_vout[v].amount, 'asset_vout', asset_guid, tx, function () {
