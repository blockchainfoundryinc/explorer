...
function update_address(hash, txid, amount, type, asset_guid, vindex, length, tx, cb) {
  // Check if address exists
  var s_timer = new Date().getTime();
  find_address(hash, function(address) {
    console.log('find address:', address, new Date().getTime() - s_timer);
    ...
    if (address) {
      ...
      if ( hash === 'coinbase' ) {
        ...
        lib.is_unique(address.txs, txid, function(unique, index) {
          ...
          switch(type) {
            ...
            case 'asset_vin':
              if(tx.systx.txtype === 'assettransfer' || tx.systx.txtype === 'assetactivate' || tx.systx.txtype === 'assetsend') {
                asset_balances[asset_guid] -= amount;
              }else if(tx.systx.txtype === 'assetallocationsend') {
                asset_allocation_balances[asset_guid] -= amount;
              }
              break;
            case 'asset_vout':
              ...
              if(asset_balances[asset_guid] === undefined && (tx.systx.txtype === 'assettransfer' || tx.systx.txtype === 'assetactivate')) {
                  asset_balances[asset_guid] = amount;
              }else if(asset_allocation_balances[asset_guid] === undefined && (tx.systx.txtype === 'assetallocationsend' || tx.systx.txtype === 'assetsend')) {
                if(tx.systx.txtype === 'assetallocationsend') {
                  asset_allocation_balances[asset_guid] = amount;
          ...
          if(asset_guid != null) {
            console.log(`AFTER UPDATE ${address.a_id}: Balances for ${asset_guid}: unallocated: ${asset_balances[asset_guid]} | allocated: ${asset_allocation_balances[asset_guid]}`);
          }

          console.log('address preprocess:', new Date().getTime() - s_timer);
          ...
          if (unique == true) {
            ...
            Address.update({a_id:hash}, {
              ...
              console.log('address UPDATE unique:', new Date().getTime() - s_timer);
            ...
            if (type == tx_array[index].type) {
              ...
              Address.update({a_id:hash}, {
                ...
                console.log('address UPDATE standard:', new Date().getTime() - s_timer);
                return saveAssetInfo(tx, asset_guid, hash, cb);
              });
            }
          }
      ...
      if(tx.systx) {
        ...
        if (tx.systx.txtype === 'assetactivate' || tx.systx.txtype === 'assettransfer') {
          newAddressAssetBalances = {[asset_guid]: amount};
        } else if (tx.systx.txtype === 'assetsend' || tx.systx.txtype === 'assetallocationsend') {
          newAddressAssetAllocationBalances = {[asset_guid]: amount};
        }
      }
      ...
      newAddress.save(function(err) {
        if (err) {
          ...
          console.log('address SAVE unique:', new Date().getTime() - s_timer);
          return saveAssetInfo(tx, asset_guid, hash, cb);
        }
      });
    }
...
function save_tx(txid, blockhash, blockheight, cb) {
  ...
  lib.get_rawtransaction(txid, blockhash, function(tx){
    console.log('getrawtx took:', new Date().getTime() - s_timer);
    if (tx != 'There was an error. Check your console.') {
      lib.prepare_vin(tx, function (vin) {
        lib.prepare_vout(tx.vout, txid, vin, tx.hex, async function (vout, nvin) {
          console.log('prepare in/out:', new Date().getTime() - s_timer);

          let assetInOut = await syscoinHelper.prepareAssetVinVout(tx);
          let asset_vin = assetInOut.vin;
          let asset_vout = assetInOut.vout;
          let txtype = "send",
            asset_guid,
            asset_total_vout_not_change = 0,
            asset_totalout = 0,
            asset_symbol = '',
            asset_publicvalue;
          ...
          if (assetTxInfo) {
            ...
            asset_totalout = asset_vout.reduce((acc, curr, index, arr) => acc + curr.amount, 0);
            console.log("ASSET TX:", txid, " TXTYPE", txtype);
            console.log("asset total out: ", asset_total_vout_not_change);
            ...
            if(asset_vin.length && asset_vout.length) {
              for(let i = 0; i < asset_vout.length - 1; i++) {
                asset_total_vout_not_change += asset_vout[i].amount;
              }

              if(asset_total_vout_not_change > asset_vin[0].amount) {
                let msg = `DOUBLE SPEND! ${tx.txid} ASSET ${asset_guid}`;
                console.log(`   ${msg}`);
                utils.errorAndExit();
                return cb();
              }
          ...
          console.log('fully prepped took:', new Date().getTime() - s_timer);

          lib.syncLoop(vin.length, function (tx_vin_loop) {
            var i = tx_vin_loop.iteration();
            update_address(nvin[i].addresses, txid, nvin[i].amount, 'vin', null, i, nvin.length, tx, function () {
              tx_vin_loop.next();
              console.log('loop1:', new Date().getTime() - s_timer);
            });
          }, function () {
            lib.syncLoop(vout.length, function (tx_vout_loop) {
              var t = tx_vout_loop.iteration();
              if (vout[t].addresses) {
                update_address(vout[t].addresses, txid, vout[t].amount, 'vout', null, t, vout.length, tx, function () {
                  tx_vout_loop.next();
                  console.log('loop2vout:', new Date().getTime() - s_timer);
                });
              } else {
                tx_vout_loop.next();
                console.log('loop2:', new Date().getTime() - s_timer);
              }
            }, function () {
              lib.syncLoop(asset_vin.length, function (asset_vin_loop) {
                var u = asset_vin_loop.iteration();
                if (asset_vin[u].address) {
                  update_address(asset_vin[u].address, txid, asset_vin[u].amount, 'asset_vin', asset_guid, u, asset_vin.length, tx, function () {
                    asset_vin_loop.next();
                    console.log('loop3:', new Date().getTime() - s_timer);
                  });
                } else {
                  asset_vin_loop.next();
                  console.log('loop3:', new Date().getTime() - s_timer);
                ...
                lib.syncLoop(asset_vout.length, function (asset_vout_loop) {
                  ...
                  if (asset_vout[v].address) {
                    update_address(asset_vout[v].address, txid, asset_vout[v].amount, 'asset_vout', asset_guid, v, asset_vout.length, tx, function () {
                      asset_vout_loop.next();
                      console.log('loop4:', new Date().getTime() - s_timer);
                    });
                  } else {
                    asset_vout_loop.next();
                    console.log('loop4:', new Date().getTime() - s_timer);
                  ...
                  lib.calculate_total(vout, function (total) {
                    ...
                    var newTx = new Tx({
                      ...
                      asset_total: txtype == 'assetactivate' || txtype == 'assetupdate' ? asset_totalout.toFixed(8) : asset_total_vout_not_change.toFixed(8),
                      vout: vout,
