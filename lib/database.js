      ...
      lib.prepare_vin(tx, function (vin) {
        lib.prepare_vout(tx.vout, txid, vin, tx.hex, async function (vout, nvin) {
          ...
          Object.keys(assetInOutUnique).forEach(key => {
            assetInOut[key].forEach(entry => {
              const uniqueEntry = assetInOutUnique[key].find(item => item.address === entry.address);
              if (uniqueEntry) {
                uniqueEntry.amount = new BigNumber(uniqueEntry.amount).plus(new BigNumber(entry.amount));
              } else {
          ...
          lib.syncLoop(vin.length, function (tx_vin_loop) {
            ...
            lib.syncLoop(vout.length, function (tx_vout_loop) {
              ...
              lib.syncLoop(assetInOutUnique.vin.length, function (asset_vin_loop) {
                var u = asset_vin_loop.iteration();
                if (assetInOutUnique.vin[u].address) {
                  update_address(assetInOutUnique.vin[u].address, txid, assetInOutUnique.vin[u].amount, 'asset_vin', asset_guid, u, assetInOutUnique.vin.length, tx, blockheight, function () {
                    asset_vin_loop.next();
                  });
                } else {
                  asset_vin_loop.next();
                ...
                lib.syncLoop(assetInOutUnique.vout.length, function (asset_vout_loop) {
                  var v = asset_vout_loop.iteration();
                  if (assetInOutUnique.vout[v].address) {
                    update_address(assetInOutUnique.vout[v].address, txid, assetInOutUnique.vout[v].amount, 'asset_vout', asset_guid, v, assetInOutUnique.vout.length, tx, blockheight, function () {
