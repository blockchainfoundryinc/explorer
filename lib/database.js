        ...
        lib.prepare_vout(tx.vout, txid, vin, tx.hex, async function (vout, nvin) {
          ...
          if (assetTxInfo) {
            ...
            if(asset_vin.length && asset_vout.length) {
              ...
              if(parseFloat(asset_total_vout_not_change.toFixed(8)) > asset_vin[0].amount) {
                let msg = `DOUBLE SPEND! ${tx.txid} ASSET ${asset_guid} because ${asset_total_vout_not_change} > ${asset_vin[0].amount}`;
                console.log(`   ${msg}`);
                utils.errorAndExit();
                return cb();
              }
