        ...
        if (block) {
          lib.prepare_vin(tx,  function(vin, asset_vin) {
            lib.prepare_vout(tx.vout, txid, vin, asset_vin, tx.hex, async function(vout, nvin, asset_vout, nasset_vin) {
              let asset_guid = await syscoinHelper.getAssetGuidFromHex(tx.hex);
              if(asset_guid) console.log("ASSET TX:", txid);

              let asset_totalout = asset_vout.reduce((acc, curr, index, arr) => acc + curr.amount, 0);
                ...
                lib.syncLoop(vout.length, function (tx_vout_loop) {
                  ...
                  lib.syncLoop(asset_vin.length, function (asset_vin_loop) {
                    ...
                    if (asset_vin[u].address) {
                      update_address(asset_vin[u].address, txid, 0, 'asset_vout', function () {
                        asset_vin_loop.next();
                      });
                    } else {
                      asset_vin_loop.next();
                    ...
                    lib.syncLoop(asset_vout.length, function (asset_vout_loop) {
                      ...
                      if (asset_vout[v].address) {
                        update_address(asset_vout[v].address, txid, 0, 'asset_vin', function () {
