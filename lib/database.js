        ...
        lib.is_unique(address.txs, txid, function(unique, index) {
          ...
          if(asset_guid !== null) {
            addressHistory = new AddressHistory({
              a_id: hash,
              txid: txid,
              balance: asset_allocation_balances[asset_guid].toString(),
              balance_change: delta.toFixed(),
              assetType: asset_guid,
              block: tx.height,
              timestamp: tx.time
            });
          }else{
            addressHistory = new AddressHistory({
              a_id: hash,
              txid: txid,
              balance: received.minus(sent).dividedBy(100000000).toFixed(),
              balance_change: delta.dividedBy(100000000).toFixed(),
              assetType: "SYS",
              block: tx.height,
              timestamp: tx.time
            });
