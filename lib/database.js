    ...
    if (address) {
      ...
      if ( hash === 'coinbase' ) {
        ...
        lib.is_unique(address.txs, txid, function(unique, index) {
          ...
          if(asset_guid != null) {
            console.log("SAVE ADDRRESS");
            console.log(`Balances for ${asset_guid}: unallocated: ${asset_balances[asset_guid]} | allocated: ${asset_allocation_balances[asset_guid]}`);
          }
          ...
          if(asset_guid != null) {
            console.log(`UPDATED Balances for ${asset_guid}: unallocated: ${asset_balances[asset_guid]} | allocated: ${asset_allocation_balances[asset_guid]}`);
          }
          ...
          if (assetTxInfo) {
            ...
            if(asset_vin.length && asset_vout.length) {
              ...
              if(asset_total_vout_not_change > asset_vin[0].amount) {
                utils.errorAndExit(`DOUBLE SPEND! ${tx.txid} ASSET ${asset_guid}`);
                return cb();
              }
            }
          }
              ...
              lib.syncLoop(asset_vin.length, function (asset_vin_loop) {
                ...
                lib.syncLoop(asset_vout.length, function (asset_vout_loop) {
                  ...
                  lib.calculate_total(vout, function (total) {
                    ...
                    var newTx = new Tx(ntx);
                    if(tx.systx)
                      console.log('NEWTX:', JSON.stringify(newTx), 'from', JSON.stringify(ntx), 'symbol is ', asset_symbol);
