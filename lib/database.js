...
function update_address(hash, txid, amount, type, asset_guid, txtype, cb) {
  console.log("Update address: ", txtype);
  ...
  find_address(hash, function(address) {
    if (address) {
        ...
        lib.is_unique(address.txs, txid, function(unique, index) {
          ...
          switch(type) {
            ...
            case 'asset_vout':
              ...
              if(txtype === "assetactivate") {
                console.log("ASSET ACTIVATE:". asset_guid, txid);
              }
      ...
      if (type === 'vin') {
        ...
        let asset_ownership = {};
        if(type === 'assetactivate') {
          asset_ownership[asset_guid] = true;
        }

        console.log("ASSIGN OWNER:", asset_guid, " TO ", hash);
      ...
      lib.prepare_vin(tx, function (vin) {
        lib.prepare_vout(tx.vout, txid, vin, tx.hex, async function (vout, nvin) {
          ...
          lib.syncLoop(vin.length, function (tx_vin_loop) {
            ...
            update_address(nvin[i].addresses, txid, nvin[i].amount, 'vin', null, txtype, function () {
              tx_vin_loop.next();
            });
          }, function () {
            lib.syncLoop(vout.length, function (tx_vout_loop) {
              ...
              if (vout[t].addresses) {
                update_address(vout[t].addresses, txid, vout[t].amount, 'vout', null, txtype, function () {
                  tx_vout_loop.next();
                });
              } else {
                tx_vout_loop.next();
              ...
              lib.syncLoop(asset_vin.length, function (asset_vin_loop) {
                ...
                if (asset_vin[u].address) {
                  update_address(asset_vin[u].address, txid, asset_vin[u].amount, 'asset_vin', asset_guid, txtype, function () {
                    asset_vin_loop.next();
                  });
                } else {
                  asset_vin_loop.next();
                ...
                lib.syncLoop(asset_vout.length, function (asset_vout_loop) {
                  ...
                  if (asset_vout[v].address) {
                    update_address(asset_vout[v].address, txid, asset_vout[v].amount, 'asset_vout', asset_guid, txtype, function () {
