...
module.exports = {
  ...
  prepare_vout: function(vout, txid, vin, asset_vin, rtx, cb) {
    var arr_vout = [];
    var arr_vin = [];
    arr_vin = vin;
    module.exports.syncLoop(vout.length, function (loop) {
      ...
      let asset_vout = (await syscoinHelper.prepareAssetVinVout(rtx)).vout;
      //console.log("asset vout:", asset_vout);
      ...
      if (vout[0].scriptPubKey.type == 'nonstandard') {
        if ( arr_vin.length > 0 && arr_vout.length > 0 ) {
          if (arr_vin[0].addresses == arr_vout[0].addresses) {
            ...
            arr_vin.shift();
            return cb(arr_vout, arr_vin, asset_vout, asset_vin);
          } else {
            return cb(arr_vout, arr_vin, asset_vout, asset_vin);
          }
        } else {
  ...
  get_input_addresses: function(input, vout, tx, cb) {
    var addresses = [];
    if (input.coinbase) {
      var amount = 0;
      module.exports.syncLoop(vout.length, function (loop) {
  ...
  prepare_vin: function(tx, cb) {
    ...
    module.exports.syncLoop(tx.vin.length, function (loop) {
      var i = loop.iteration();
      module.exports.get_input_addresses(tx.vin[i], tx.vout, tx.vin[i].coinbase? null: tx, function(addresses){
      ...
      //index asset data
      let arr_asset_vin = (await syscoinHelper.prepareAssetVinVout(tx.hex)).vin;
      return cb(arr_vin, arr_asset_vin);
    });
  }
};
