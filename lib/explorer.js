...
module.exports = {
  ...
  get_rawtransaction: function(hash, blockhash, cb) {
    var uri = base_url + 'getrawtransaction?txid=' + hash + '&verbose=1';
    //TODO when the RPC supports blockhash (in sys4/latest bitcoin) reinstate this code
    //if(blockhash != null){
    //  uri += '&blockhash=' + blockhash;
    //}
    request({uri: uri, json: true}, function (error, response, body) {
      return cb(body);
    });
  ...
  send_rawtransaction: function(hex, cb) {
    var uri = base_url + 'sendrawtransaction?hex=' + hex;
    request({uri: uri, json: true}, function (error, response, body) {
      return cb(body);
    });
  },
  ...
  is_unique: function(array, comparisonObj, cb) {
    var unique = true;
    var index = null;

    module.exports.syncLoop(array.length, function (loop) {
      var i = loop.iteration();
      if (array[i].addresses == comparisonObj) {
        unique = false;
        index = i;
        loop.break(true);
        loop.next();
  ...
  prepare_vout: function(vout, txid, vin, rtx, cb) {
    var arr_vout = [];
    var arr_vin = [];
    arr_vin = vin;
    module.exports.syncLoop(vout.length, async function (loop) {
      var i = loop.iteration();
      ...
      if (vout[i].scriptPubKey.type != 'nonstandard' && vout[i].scriptPubKey.type != 'nulldata') {
        ...
        let address;
        if (vout[i].scriptPubKey.type === 'pubkey') {
          let decodedScript = await syscoinHelper.decodeScript(vout[i].scriptPubKey.hex);
          let p2pkh = await syscoinHelper.convertAddress(decodedScript.segwit.addresses[0]);
          address = p2pkh.v3address;
          console.log('vout:' + i + ' : ' + txid + ' : ' + vout[i].scriptPubKey.type + ' : ' + address);
        } else {
      ...
      } else if (vout[0].scriptPubKey.asm.indexOf("OP_RETURN") > -1 && vout[0].value > 0) { //syscoinburn
        module.exports.convert_to_satoshi(parseFloat(vout[0].value), function(amount_sat){
          arr_vout.unshift({addresses: 'syscoinburn', amount: amount_sat, opreturn: vout[0].scriptPubKey.asm});
          return cb(arr_vout, arr_vin);
        });
  ...
  get_input_addresses: function(input, vout, cb) {
    ...
    if (input.coinbase) {
      ...
      module.exports.get_rawtransaction(input.txid, null, function(tx){
        if (tx) {
